...
function checkVal($string)
{
    $val=true;
    $string = trim($string);
if (!ctype_digit($string) && !empty($string)) {
    $val=false;
} 
return $val;
}
...
<a href='https://medclaimassist.co.za/login/'>Click here to login</a><br><br>Yours sincerely<br>The Med ClaimAssist Team and Angela Van Breda";
             ...
             elseif((int)$_SESSION['mca_user_id']==985)
        {
            $mail->Body = "<span style='font-family: Arial, sans-serif;font-size: 10px;'>Dear $name <br><br>Welcome to the RFAdvice and Med ClaimAssist system.<br><br> 
...
<br><br></span>
<img src='https://medclaimassist.co.za/admin/PHPExcel/color_logo_transparent_background_small.png'/>&ensp;&ensp;&ensp;
...
function retrieveData($fileName)
{
    ...
    for($row=2;$row<=$lastRow;$row++)
    {
        ...
        if(!checkVal($contact_number))
        {
            die("Invalid contact number, seems like you are you using incorrect excel template.<hr>");
        }
        if(!checkVal($medical_aid_number))
        {
            die("Invalid medical aid number, seems like you are you using incorrect excel template.<hr>");
        }
        $email = trim($email);
...
medical_aid_number, physical_address1, physical_address2, entered_by,role,broker_id,password,account_holder,account_number,insurer,option_in) 
