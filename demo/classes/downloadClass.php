...
//$html = "<b style='font-weight: bolder'>".'Year-Month' . "\t" . 'Claims Closed' . "\t" . 'Savings by Dr Discount' . "\t" . 'Savings by Scheme Paid' . "\t" . 'Total Savings' . "\t" . 'Value of Claims Referred' . "\t" . 'Percentage saved (of total referred)' . "\t" . 'Average time to close (days)' . "\t" . 'Claims Referred'  . "<b>" . "\n";
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('I1', 'Value of Claims')
        ->setCellValue('J1', 'Savings')
        ->setCellValue('K1', 'Percentage');
    $from = "A1";
    $to = "K1";
    $objPHPExcel->getActiveSheet()->getStyle("$from:$to")->getFont()->setBold(true);
    $rowCount = 2;
...
//$savings=number_format($savings,2,',','');
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('I' . $rowCount, $claims_value);
                $objPHPExcel->getActiveSheet()->SetCellValue('J' . $rowCount, $savings);
                $objPHPExcel->getActiveSheet()->SetCellValue('K' . $rowCount, $perc);
                $rowCount++;
            }
...
elseif (isset($_POST["mysnap"]))
{
    include ("../classes/reportsClass.php");
    $results=new reportsClass();
    $frmdate=$_POST["from_client"]."-01";
    $todate=$_POST["to_client"]."-31 23:59:59";
    $objPHPExcel = new PHPExcel();
// Set document properties
    $objPHPExcel->getProperties()->setCreator("Govinda")
        ->setLastModifiedBy("Govinda")
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ->setCellValue('A1', 'Claim Number')
        ->setCellValue('B1', 'Charged Amount')
        ->setCellValue('C1', 'Scheme Paid')
        ->setCellValue('D1', 'Gap')
        ->setCellValue('E1', 'Scheme Savings')
        ->setCellValue('F1', 'Discount Savings')
        ->setCellValue('G1', 'Date Closed');
    $from = "A1";
    $to = "G1";
    $objPHPExcel->getActiveSheet()->getStyle("$from:$to")->getFont()->setBold(true);
    $rowCount = 2;
    try {
        foreach ($results->getSnapData($frmdate,$todate) as $row1)
        {            
                $claim_number=$row1["claim_number"];
                $charged_amount=$row1["charged_amount"];
                $scheme_paid=$row1["scheme_paid"];
                $gap=$row1["gap"];
                $scheme_savings=$row1["scheme_savings"];
                $discount_savings=$row1["discount_savings"];
                $date_closed=$row1["date_closed"]; 
                $objPHPExcel->getActiveSheet()->SetCellValue('A' . $rowCount, $claim_number);
                $objPHPExcel->getActiveSheet()->SetCellValue('B' . $rowCount, $charged_amount);
                $objPHPExcel->getActiveSheet()->SetCellValue('C' . $rowCount, $scheme_paid);
                $objPHPExcel->getActiveSheet()->SetCellValue('D' . $rowCount, $gap);
                $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $scheme_savings);
                $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $discount_savings);
                $objPHPExcel->getActiveSheet()->SetCellValue('G' . $rowCount, $date_closed);
                $rowCount++;           
...
// Rename worksheet
    $objPHPExcel->getActiveSheet()->setTitle("Snap Report");
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
    $objPHPExcel->setActiveSheetIndex(0);

    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
    header("Content-disposition: attachment; filename=Snap-Report.xls");
    header('Cache-Control: max-age=0');
// If you're serving to IE 9, then the following may be needed
    header('Cache-Control: max-age=1');
...
// Redirect output to a clientÃÂÃÂ¢ÃÂÃÂÃÂÃÂs web browser (Excel5)
    header('Content-Type: application/vnd.ms-excel; charset=UTF-8');
...
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
    ...
    try {
        foreach($arr as $row) {
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue('N' . $rowCount, $client_name1);
