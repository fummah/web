...
a.Open, a.claim_id, a.username, a.savings_discount, b.scheme_number,a.sla,c.client_name,a.pmb,a.charged_amnt,a.scheme_paid,a.entered_by,a.emergency,a.quality";
    ...
    public function getClaimHeader($claim_id)
    {
       ...
       b.broker,c.client_name,d.patient_name,c.client_email,d.patient_address,d.patient_contact FROM claim as a INNER JOIN member as b ON a.member_id=b.member_id INNER JOIN clients as c ON b.client_id=c.client_id LEFT JOIN patient as d ON a.claim_id=d.claim_id WHERE a.claim_id=:claim_id");
  ...
  public function getClaimDoctors($claim_id)
    ...
    {
        $stmt = $this->conn->prepare("SELECT *FROM decline_reasons");        
        $stmt->execute();
...
GROUP BY k.claim_id");
    ...
    function getClosedThisMonth($condition,$val)
    {
        $dat = date('Y-m')."-01";
        $dat2 = $this->nexDate($dat);
        $stmt = $this->conn->prepare('SELECT COUNT(Open) as allOpen FROM claim as a INNER JOIN member as b ON a.member_id=b.member_id INNER JOIN clients as c ON b.client_id=c.client_id WHERE a.date_closed >= :dat AND a.date_closed < :dat2 AND a.Open=0 AND '.$condition);
        $stmt->bindParam(':dat', $dat, PDO::PARAM_STR);
        $stmt->bindParam(':dat2', $dat2, PDO::PARAM_STR);
        $stmt->bindParam(':username', $val, PDO::PARAM_STR);
        $stmt->execute();
    ...
    function getEnteredthisClaims($date,$condition,$val)
    {
        try {
            $dat = $date."-01";
            $dat2 = $this->nexDate($dat);
            $stmt = $this->conn->prepare("SELECT count(*) FROM `claim` as a INNER JOIN member as b ON a.member_id=b.member_id INNER JOIN clients as c ON b.client_id=c.client_id WHERE a.date_entered >= :dat AND a.date_entered < :dat2 AND a.Open<>2 AND ".$condition);
            $stmt->bindParam(":dat",$dat,PDO::PARAM_STR);
            $stmt->bindParam(":dat2",$dat2,PDO::PARAM_STR);
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->execute();
...
ON a.member_id=b.member_id INNER JOIN clients as c ON b.client_id=c.client_id WHERE a.date_closed >= :dat AND a.date_closed < :dat2 AND a.Open=0 AND a.Open=0 AND a.claim_id not in (SELECT claim_id FROM reopened_claims) AND $condition";
    ...
    function getOpenNew($condition=":username",$val="1")
    {
        $note="This claim was sent for clinical review.";
        try {
 ...
 function getSubscription($email)
    {
        $broker_name=array("fullname"=>"No Broker","insurer"=>"");
        $fbStmt = $this->conn->prepare("SELECT broker_id,insurer FROM `web_clients` where email=:email");
        $fbStmt->bindParam(':email', $email, PDO::PARAM_STR);
        $fbStmt->execute();
        if($fbStmt->rowCount()>0)
        {
            $aarb=$fbStmt->fetch();
            $broker_id=(int)$aarb["broker_id"];
            $insurer=$aarb["insurer"];
            ...
            {
                $broker_name=array("fullname"=>$fbStmt1->fetchColumn(),"insurer"=>$insurer);
            }
            else{
                $broker_name=array("fullname"=>"No Broker","insurer"=>"");
            }
...
function getAPIURL($sender_id)
  ...
  function updatePatient($claim_id,$key,$value)
    {
        $stmt=$this->conn->prepare('UPDATE patient SET '.$key.'=:val WHERE claim_id=:claim_id');
        $stmt->bindParam(':claim_id', $claim_id, PDO::PARAM_STR);       
  ...
  signed, date_joined,entered_by,"'.$username.'" FROM doctor_details WHERE doc_id=:doctor_id');
    ...
    function getMonthlySavings($date,$condition=":username",$val="1")
    {
        try {
            ...
            $dat = $date."-01";
            $dat2 = $this->nexDate($dat);
            $stmt = $this->conn->prepare("SELECT SUM(a.savings_scheme + a.savings_discount) as sav,a.charged_amnt,a.scheme_paid,SUM(a.savings_scheme) as savings_scheme,SUM(a.savings_discount) as savings_discount FROM `claim` as a  WHERE Open=0 AND a.date_closed >= :dat AND a.date_closed < :dat2 AND ".$condition);
            $stmt->bindParam(":dat",$dat,PDO::PARAM_STR);
            $stmt->bindParam(":dat2",$dat2,PDO::PARAM_STR);
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->execute();
   ...
   function getValidations()
    {
        $stmt =$this->conn->prepare("SELECT id,rule_name,description,vals FROM internal_rules ORDER BY id ASC");        
        $stmt->execute();
        return $stmt->fetchAll();
    }
function getValidationsInd($id)
    {
        try
        {
        $stmt =$this->conn->prepare("SELECT vals FROM internal_rules WHERE id=:id"); 
        $stmt->bindParam(':id', $id, PDO::PARAM_STR);       
        $stmt->execute();
        return $stmt->fetchColumn();
        }
   ...
   public function getSplitTotals($startdate,$enddate,$val)
    {
        ...
        if(!empty($startdate))
        {
            $enddate = $this->nexDate1($enddate);
            $condition.=$val=="completed"?"AND b.date_closed>=:dat1 AND b.date_closed<:dat2":"AND b.date_entered>=:dat1 AND b.date_entered<:dat2";
...
//echo $sql."----".$val."----".$startdate."======".$enddate;
    ...
    public function getSplitGraph1($startdate,$enddate,$val)
    {
        ...
        if(!empty($startdate))
        {
            $condition.=$val=="completed"?"AND date_closed>=:dat1 AND date_closed<:dat2":"AND date_entered>=:dat1 AND date_entered<:dat2";
        }
    ...
    public function downloadSplitCompleted($startdate,$enddate,$val)
    {
        $condition="WHERE 1 ";
        if($val!="all")
        {
        ...
        {
            $enddate = $this->nexDate1($enddate);
            $condition.="AND a.date_closed>=:dat1 AND a.date_closed<:dat2";
        }

        $sql="SELECT a.id as claim_id,b.id as member_id,loyalty_number,a.file_name,membership_number,beneficiary_name,
       beneficiary_scheme_join_date,beneficiary_id_number,beneficiary_date_of_birth,co_payment,discharge_date,admission_date,procedure_date,a.claim_number,a.date_closed,a.date_entered,a.status,a.closed_by FROM split_claim as a INNER JOIN split_member as b ON a.split_member_id=b.id ".$condition." GROUP BY a.date_closed";
 ...
 function updateSFTPNegative()
    {
        $stmt =$this->conn->prepare("UPDATE split_claim SET archived_at=NOW(),date_closed=NOW(),status='completed',closed_by='Archive' WHERE id IN(SELECT id FROM `split_claim` where status='pending' and id not in (SELECT split_claim_id from split_claim_line WHERE amountcharged>0))");
        return $stmt->execute();         
    }
 ...
 function getPendingQAUsers($dat)
  ...
  function updateQAKey($claim_id,$key,$value)
    ...
    function nexDate1($dat)
    {
        return date('Y-m-d', strtotime('+1 Day', strtotime($dat)));
    }
