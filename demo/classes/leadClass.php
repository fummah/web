...
//This claim was sent for clinical review.
    function sendMail($email,$first_name,$lead_id)
    {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            //Server settings
            //$mail->SMTPDebug = 2;
            $body="Dear $first_name<br><br>Med ClaimAssist has evaluated your request and it's possible that we may be able to address the relevant stakeholders in order to get to a positive outcome.If you would like us to proceed with your request, you may subscribe to any one of our services.<br><br><a href='https://medclaimassist.co.za/product/annually'>Med ClaimAssist Subscription.</a><br><br>Yours Sincerely,<br>The Med ClaimAssist Team";
...
VALUES (:member_id,:claim_number, :Service_Date,:icd10,:pmb, :charged_amnt, :scheme_paid, :gap,:username, :emergency, :entered_by,:end_date,:client_gap,:category_type,:medication_value,:patient_dob,:fusion_done,:code_description,:modifier,:reason_code,:contact_person_email,:patient_gender,:patient_idnumber)');
    ...
    {
        return strtr($string,"???????ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¿","SOZsozYYuAAAAAAACEEEEIIIIDNOOOOOOUUUUYsaaaaaaaceeeeiiiionoooooouuuuyy");
    }
