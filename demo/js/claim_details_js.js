...
// $("#alert1").html("There is an error : "+jqXHR.responseText);
    ...
    function addNotes(claim_id,sla=0){
        ...
        if(em=="1" || em=="0") {
            ...
            let reason_id=$("#reason").val();
            let description = $("#reason option:selected").text();            
            //test
            $("#"+vasid).attr("isreason",isreason);
            $("#"+vasid).attr("reason_id",reason_id);
            $("#"+vasid).attr("description",description);
...
//  alert("Record Successfully Saved");
    ...
    function highlitDoctor(practice_number,doc_name,zesthid) {
        ...
        var xcx=$("#"+pydid).text();
        let isreason=$("#"+vasid).attr("isreason");
        let reason_id=$("#"+vasid).attr("reason_id");       
        ...
        $("#doc_vas").val(txt_vas);
        //console.log(reason_id+"==="+description);
        //$('.reason').prepend("<option value='"+reason_id+"'>"+description+"</option>");
        $("#reason").val(reason_id);
        ...
        if(isreason=="yes")
        {
            document.getElementById("scheme_declined1").checked = true;
            $(".te").show();
        }
        else if(isreason=="no")
        {
            document.getElementById("scheme_declined2").checked = true;
            $(".te").hide();
        }
        else
        {
            document.getElementById("scheme_declined1").checked = false;
            document.getElementById("scheme_declined2").checked = false;
            $(".te").hide();
        }
...
//$('#eightShow').hide();
  ...
  function clientAmounts(){
        let claim_id=$("#docclaim_id").val();
        let practice_number=$("#docpractice_number").text();
        let charged_amount=$("#doccharged_amount").val();
        let scheme_amount =$("#docscheme_amount").val();
        let gap=$("#docgap").val();
        let obj={claim_id,practice_number,charged_amount,scheme_amount,gap,identity_number:47};
           $.ajax({
            url:"ajax/claims.php",
            type:"POST",
            data:obj,
