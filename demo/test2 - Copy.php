...
function getID($claim_number)
{
    global $conn;

    $stmt=$conn->prepare('SELECT claim_id FROM claim WHERE claim_number=:claim_number');
...
<head>
    ...
    <script src="../admin/uikit/js/uikit.min.js"></script>
    <script src="../admin/uikit/js/uikit-icons.min.js"></script>
    <style>
        .bttn{
...
if(isset($_SESSION['mca_logxged']) && !empty($_SESSION['mca_logxged']) && $_SESSION['mca_role']=="broker") {
    ...
    if (isset($_POST['upload_now']) && !empty($_POST['area1'])) {
        ...
        try {
            ...
            if($ccc>0) {
                ...
                if($_SESSION['mca_user_id']==$broker_id) {
                    $mcaclient_id = 4;
                    $open = 1;
 ...
 :scheme_option,:broker)');
                            $insertMember->bindParam(':client_id',$mcaclient_id,PDO::PARAM_INT);
                            ...
                            if(addClaim($member_id,$claim_number,$myusername))
                            {
                                ...
                                <p class="alert alert-success alert-dismissible" align="center"
                                   style="padding-right:25px;padding-left: 25px">
                                    ...
                                    <strong>Success!</strong> New Claim created.

                                </p>

                                <?php
                                echo "<div style='display: block; text-align: center;'>";
                                echo "<form action='view_claim.php' method='post' style='display: inline-block;margin-left: auto; margin-right: auto;text-align: left;'/>";
                                echo "<input type=\"hidden\" name=\"claim_id\" value=\"$xclaim_id\" />";
                                echo "<input type=\"hidden\" name=\"username\" value=\"$myusername\" />";
                                echo "<button class=\"uk-button uk-button-default\" name='btn'>View Claim</button>";
                                echo "</form>";
                                echo "</div>";
                    ...
                    else
                    {
                        ...
                        <p class="alert alert-danger alert-dismissible" align="center"
                           style="padding-right:25px;padding-left: 25px">
                            <button type="button" class="close" data-dismiss="alert">&times;</button>
                            <strong>Failed!</strong> This client has lot of open claims.
                        </p>
                        <?php
                    }
                }
    ...
    <h3 align="center" class="hd">Log a Query/Claim for <b><?php echo $fullname;?></b></h3>
    <div class="container" >
