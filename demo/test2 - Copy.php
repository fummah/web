...
class jv_import_export
{
    ...
    function checkClaim1($claim_number,$client_id)
    {
        global $conn;
        $try=false;
        $checkM=$conn->prepare('SELECT claim_id FROM claim as a INNER JOIN member as b ON a.member_id=b.member_id WHERE a.claim_number=:claim_number AND b.client_id=:client_id LIMIT 1');
        $checkM->bindParam(':claim_number', $claim_number, PDO::PARAM_STR);
        $checkM->bindParam(':client_id', $client_id, PDO::PARAM_STR);
        $checkM->execute();
        $cc=$checkM->rowCount();
    ...
    function readFile()
    {
        ...
        for($i=1;$i<$array_count;$i++) {
            ...
            try {
                ...
                $continue=true;
                $client_id = 16;
                $western=["AUHGAP","AGAGG","AUHGCOMP"];
                $kaelo=["CENCAN","CENKGP","DCHDHC"];
                $sanlam=["CENSAN"];
                if(in_array($product_code,$western))
                {
                    $client_id = 16;
                }
                elseif (in_array($product_code,$kaelo))
                {
                    $client_id = 3;
                    $owner="Kaelo";
                }
                elseif (in_array($product_code,$sanlam))
                {
                    $client_id = 15;
                    $owner="Sanlam";
                }
                ...
                $r_s = false;
                $today = date("Y-m-d");
                if(in_array($owner,$clients_arr) && $today>"2024-04-01")
                {
                    $r_s = true;
                }
                        ...
                        for ($d=0;$d<$countdoctors;$d++)
                        {
                            ...
                            if($chhh)
                            {
                                ...
                                for($j=0;$j<$countLine;$j++) {
                                    ...
                                    $lastUpdateDate = "";
                                    $toothNo = "";
                                    $selectClaimline = $conn->prepare('SELECT mca_claim_id FROM claim_line WHERE primaryICDCode=:icd AND tariff_code=:tariff_code AND treatmentDate=:treatmentDate AND treatmentType=:treatmentType AND clmnline_charged_amnt=:clmnline_charged_amnt AND clmline_scheme_paid_amnt=:clmline_scheme_paid_amnt AND gap=:gap AND mca_claim_id=:mca_claim_id AND practice_number=:practice_number');
...
charged_amnt as claimChargedAmnt,scheme_paid as schemePaidAmnt,gap as claimCalcAmnt,memberLiability FROM claim LIMIT 2');
 ...
 gap as clmlineCalcAmnt,memberLiability,benefit_description as benefitDescription,PMBFlag,clmn_line_pmnt_status as clmnLinePmntStatus,clmn_line_status_date as clmnLinePmntStatusDate,clmnLinePmntStatusBy FROM claim_line LIMIT 2');
    ...
    {
        global $conn1;
        $stmt=$conn1->prepare('select username,password,other_name,environment from staff_users where username=:user1 AND role_value=2');
        ...
        if($cc==1)
        {
            $vv=$stmt->fetch();
            $pass=$vv[1];
            if(password_verify($password,$pass) && $enviro==$vv[2] && $othername==$vv[3])
            {
                $ret=true;
            }
            else{
