        ...
        try {
            ...
            if($ccc>0) {
                ...
                if($_SESSION['mca_user_id']==$sys_id) {
                    ...
                    $dup = $conn->prepare('SELECT a.Open FROM claim as a inner join member as b ON a.member_id=b.member_id WHERE b.email=:number AND a.Open=1');
                    ...
                    if($opendup<1) {
                        ...
                        $checkMember_stmt=$conn->prepare('SELECT member_id,entered_by FROM member WHERE ((email=:email AND email<>"") OR (id_number=:id_number AND id_number<>"") AND client_id=4) LIMIT 1');
