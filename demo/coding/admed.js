...
loadData()
{
    ...
    let obj={identity_number:3,columns:columns,start_date:start_date,end_date:end_date,hierach:hierach};
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $.each(top, function(index) {
                    ...
                    if(field_name==="Claim_Identifier" || table==="x")
                    {
                        return true;
                    }
...
let objClass=new Proc();
    ...
    function rearrange(arr=[],year="")
    {
        $(".myTable").trigger('destroy');
        var tt=$("#tt");
        tt.empty();
        tt.append("<th width='30%'>"+year+"</th>");
        for(let i=0;i<arr.length;i++)
        {
            let item=arr[i];
            item=item=="id"?"Total_Claims":item;
    ...
    function addTree(count,name)
    {
        ...
        var end_date=$("#dat2").val();
        let columns=JSON.stringify(Proc.myvar);
        let obj={identity_number:5,columns:columns,start_date:start_date,end_date:end_date,hierach:hierach_str,counter:count,name:name};
...
const orderByYear = () => {
    let columns=JSON.stringify(Proc.myvar);
    let obj = {identity_number:1,columns,columns};
    rearrange(Proc.myvar,"Year");
    $.ajax({
        url: "admedajax.php",
        beforeSend:function (xhr)
        {
        ...
        success: function(data){
            ...
            for(let key =0; key<dataArray.length;key++)
            {
                ...
                {
                    let num = i==0?dataArray[key][i]:fomata(parseFloat(dataArray[key][i]).toFixed(2));
                    cell += "<td>"+num+"</td>";
                }
                row +="<tr>"+cell+"</tr>";
                y++;
