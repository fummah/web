...
$stmt = $conn->prepare('SELECT Count(*) as a FROM claim WHERE Open=0 AND '.$condition);
...
$conn=connection("doc","doctors");
  $sql = $conn->prepare("SELECT Count(*) as a FROM details");  
$sql->execute();
...
return $tot6;
 ...
 function totDoctors()
    ...
    $username=$_SESSION['user_id'];
        require_once('dbconn.php');
$conn=connection("doc","doctors");
$stmt = $conn->prepare('SELECT Count(*) as a FROM details');
$stmt->execute();
$tot6=0;
