...
`tariff_code`=:tariff_code, `primaryICDCode`=:primaryICDCode,`PMBFlag`=:PMBFlag,`benefit_description`=:ben,`benefit_description`=:ben,clmn_line_pmnt_status=:clmn_line_pmnt_status,treatmentDate=:treatmentDate,`msg_code`=:code,`lng_msg_dscr`=:lng,`msg_dscr`=:shrt,`gap_aamount_line`=:gap_aamount_line WHERE id=:id";
    ...
    elseif($identity==43)
    {
        try {
            ...
            $open=1;
            $fuma=(int)$data[1];
            if(getNote($claim_id,$intervention_desc)>0 && ((int)$data[1]==34 || (int)$data[1]==35))
            {
                $open=4;
            }

            $stmupd=$conn->prepare("UPDATE claim SET Open=:open1,username=:username WHERE claim_id=:claim_id");
            $stmupd->bindParam(':claim_id', $claim_id, PDO::PARAM_STR);
            $stmupd->bindParam(':open1', $open, PDO::PARAM_STR);
            $stmupd->bindParam(':username', $username1, PDO::PARAM_STR);
            $rt=$stmupd->execute();
            //echo $fuma."--".getNote($claim_id,$intervention_desc)."<-----".$open;
            if($rt==1)
            {
                if($rr=="zzz")
                {
...
function getNote($claim_id,$intervention_desc)
{
    global $conn;
