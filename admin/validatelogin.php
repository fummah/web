        ...
        else
        {
            ...
            if($valid==99)
            {
                ...
                try
                {
                    ...
                    $stmt = $dbh->prepare("SELECT role,role_value,user_id,expiry_date,password,fullName,username,mygap FROM staff_users 
                    ...
                    /*** if we do have a result, all is well ***/
                        ...
                        if($role[0]=="claims_specialist")
                        {
                            $message = 'Not allowed to use the link';
                        }
                        elseif ($role[4] == $md5_password || password_verify($password, $role[4])) {
                            // session_start();
                            $username=$role[6];
                            ...
                            if ($dat2 < $date1) {
                                $message = "Your password has expired please <a href='mca_change_pass.php' style='color: green'>Click Here</a> to change";
                            } else {
                                ///Update password.
