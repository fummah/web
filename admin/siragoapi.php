                ...
                if($member_id>0)
                {
                    ...
                    if(empty($claim_id)) {
                        ...
                        if ($cc1 == 1) {
                            ...
                            $this->InsertFeedback($claim_id,$description,$owner);
                        }
                        else {
                        ...
                        //Doctors Information
                    {
                        ...
                        for ($d=0;$d<$countdoctors;$d++)
                        {
                            ...
                            if(!$chhh) {
                                $insertDoctor = $conn->prepare('INSERT INTO doctors(claim_id,practice_number,claimedline_id,doc_gap) VALUES(:claim_id,:practice_number,:claimedline_id,:doc_gap)');
...
charged_amnt as claimChargedAmnt,scheme_paid as schemePaidAmnt,gap as claimCalcAmnt,memberLiability FROM claim LIMIT 2');
 ...
 gap as clmlineCalcAmnt,memberLiability,benefit_description as benefitDescription,PMBFlag,clmn_line_pmnt_status as clmnLinePmntStatus,clmn_line_status_date as clmnLinePmntStatusDate,clmnLinePmntStatusBy FROM claim_line LIMIT 2');
    ...
    {
        global $conn1;
        $stmt=$conn1->prepare('select username,password from staff_users where username=:user1 AND role_value=0');
