...
$role=auth()->user()->role;
$action = ""; 
if(strpos($role, 'CRM') !== false || $role == "Admin")   
{
    $action .= " <a type='button' href='customer_details/$id' class='btn btn-danger btn-icon btn-sm' title='View Details'><i class='now-ui-icons design_bullet-list-67'></i></a>";
}
if(strpos($role, 'Ordinary') !== false || $role == "Admin")   
{
    $action .= " <a type='button' href='subscriber_details/$id' class='btn btn-warning btn-icon btn-sm' title='View Campaigns'><i class='now-ui-icons design_bullet-list-67'></i></a>";
}
   $data[] = array( 
      ...
      "actions"=>"<a type='button' href='#' data='$id'  class='btn btn-success btn-icon btn-sm edit-customer' title='Edit'><i class='now-ui-icons design-2_ruler-pencil'></i></a> $action",     
   );
}
