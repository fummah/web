...
class leadClass
{
    ...
    function getPreassessed($role,$username)
    {
        ...
        $stmt1 = $conn->prepare("SELECT a.claim_id,b.first_name,b.surname,a.claim_number,a.username,a.preassessor,c.client_name,a.date_entered FROM claim as a INNER JOIN member as b ON a.member_id=b.member_id INNER JOIN clients as c ON b.client_id=c.client_id WHERE Open=5 AND $condition ORDER  BY a.date_entered ASC");
    ...
    function addClaim($first_name,$last_name,$email,$contact_number,$scheme_name,$scheme_number,$username,$charged_amnt,$lead_id,$descrip="")
    {
 ...
 $r=$this->insertNotes($claim_id,$descrip,$entered_by,"0000-00-00 00:00:00",0,"","","","",1);              
    ...
    function getclinical()
    {
        ...
        $stmt1 = $conn->prepare('SELECT a.claim_id,b.first_name,b.surname,a.claim_number,a.username,k.date_entered,a.gap,a.client_gap FROM claim as a INNER JOIN intervention as k ON a.claim_id=k.claim_id INNER JOIN member as b ON a.member_id=b.member_id WHERE Open=4 AND k.intervention_desc=:nnot ORDER BY k.date_entered ASC');
...
//This claim was sent for clinical review.
    ...
    function getNewQuality($status,$role="admin",$username="")
    {
        ...
        $sql="SELECT DISTINCT a.claim_id,b.first_name,b.surname,a.claim_number,b.policy_number,a.username,c.client_name FROM claim as a INNER JOIN member as b ON a.member_id=b.member_id INNER JOIN clients as c ON b.client_id=c.client_id WHERE quality=1 AND $condition";
        if($status==1)
        {
        ...
        //echo "Testing, please ignore -- ".$sql;
    ...
    function getmemberDetails($claim_id)
    {
        try {
            global $conn;
            $stmt = $conn->prepare("SELECT b.first_name,b.surname,a.claim_number,b.policy_number,a.username,a.quality FROM claim as a INNER JOIN member as b ON a.member_id=b.member_id WHERE a.claim_id=:claim_id");
            $stmt->bindParam(":claim_id",$claim_id,PDO::PARAM_STR);
            $stmt->execute();
            if($stmt->rowCount()>0)
            {
...
VALUES (:member_id,:claim_number, :Service_Date,:icd10,:pmb, :charged_amnt, :scheme_paid, :gap,:username, :emergency, :entered_by,:end_date,:client_gap,:category_type,:medication_value,:patient_dob,:fusion_done,:code_description,:modifier,:reason_code,:contact_person_email,:patient_gender,:patient_idnumber)');
    ...
    {
        return strtr($string,"???????ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¿","SOZsozYYuAAAAAAACEEEEIIIIDNOOOOOOUUUUYsaaaaaaaceeeeiiiionoooooouuuuyy");
    }
