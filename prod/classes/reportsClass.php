...
GROUP BY k.claim_id");
    ...
    function getAverageList($date,$condition=":username",$val="1")
    {
        try {
            ...
            $stmt = $conn->prepare("SELECT count(*) as cc,SUM(charged_amnt-scheme_paid) as total_val FROM `claim` as a WHERE date_entered like :dd AND Open<>2 AND ".$condition);
            $stmt->bindParam(":dd",$se,PDO::PARAM_STR);
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->execute();
            $dat=$stmt->fetch();
...
function getMyRecentClaims($last_month,$this_month,$condition=":username",$val="1")
    {
        try {
            ...
            $dd1="%".$last_month."%";
            $dd2="%".$this_month."%";
            //echo "SELECT count(*) FROM `claim` as a  WHERE date_entered like $dd1 $condition = $val";
            $stmt = $conn->prepare('SELECT count(*) FROM `claim` as a  WHERE date_entered like :dd AND '.$condition);
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->bindParam(':dd', $dd1, PDO::PARAM_STR);
            $stmt->execute();
            $last_month=$stmt->fetchColumn();

            $stmt1 = $conn->prepare('SELECT count(*) FROM `claim` as a  WHERE date_entered like :dd AND '.$condition);
            $stmt1->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt1->bindParam(':dd', $dd2, PDO::PARAM_STR);
            $stmt1->execute();
    ...
    function getMyRecentSavings($last_month,$this_month,$condition=":username",$val="1")
    {
        try {
            ...
            $dd1="%".$last_month."%";
            $dd2="%".$this_month."%";
            $stmt = $conn->prepare('SELECT SUM(savings_scheme+savings_discount) as total FROM `claim` as a  WHERE date_closed like :dd AND Open=0 AND '.$condition);
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->bindParam(':dd', $dd1, PDO::PARAM_STR);
            $stmt->execute();
            $last_month=$stmt->fetchColumn();

            $stmt1 = $conn->prepare('SELECT SUM(savings_scheme+savings_discount) FROM `claim` as a  WHERE date_closed like :dd AND Open=0 AND '.$condition);
            $stmt1->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt1->bindParam(':dd', $dd2, PDO::PARAM_STR);
            $stmt1->execute();
            $this_month=$stmt1->fetchColumn();
    ...
    function getSchemeSummary($date,$condition=":username",$val="1")
    {
        try {
            global $conn;           
           $date="%".$date."%";          
            $stmt = $conn->prepare('SELECT b.medical_scheme,COUNT(DISTINCT a.claim_id)  as total FROM `claim` as a INNER JOIN member as b ON a.member_id=b.member_id WHERE a.date_entered like :date_entered AND '.$condition.' GROUP BY b.medical_scheme ORDER BY total DESC LIMIT 5');
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->bindParam(':date_entered', $date, PDO::PARAM_STR);           
            $stmt->execute();
            return $stmt->fetchAll();
        }
        catch (Exception $e)
    ...
    function getSchemeOption($medical_scheme,$date,$condition=":username",$val="1")
    {
        try {
            global $conn;           
           $date="%".$date."%";          
            $stmt = $conn->prepare('SELECT b.scheme_option,COUNT(DISTINCT a.claim_id)  as total FROM `claim` as a INNER JOIN member as b ON a.member_id=b.member_id WHERE a.date_entered like :date_entered AND b.medical_scheme=:medical_scheme AND '.$condition.' GROUP BY b.scheme_option ORDER BY total DESC LIMIT 1');
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->bindParam(':date_entered', $date, PDO::PARAM_STR);   
      ...
      function getTariffSummary($date,$condition=":username",$val="1")
    {
        try {
            global $conn;           
           $date="%".$date."%";          
            $stmt = $conn->prepare('SELECT l.tariff_code,COUNT(DISTINCT a.claim_id) as total FROM claim_line as l INNER JOIN `claim` as a ON l.mca_claim_id=a.claim_id WHERE a.date_entered like :date_entered AND '.$condition.' GROUP BY l.tariff_code ORDER BY total DESC LIMIT 5');
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->bindParam(':date_entered', $date, PDO::PARAM_STR);           
            $stmt->execute();
            return $stmt->fetchAll();
    ...
    function getICD10Summary($date,$condition=":username",$val="1")
    {
        try {
            global $conn;           
           $date="%".$date."%";  

            $stmt = $conn->prepare('SELECT l.primaryICDCode,c.shortdesc,COUNT(DISTINCT a.claim_id) as total FROM claim_line as l INNER JOIN `claim` as a ON l.mca_claim_id=a.claim_id INNER JOIN coding as c ON l.primaryICDCode=c.diag_code WHERE a.date_entered like :date_entered AND '.$condition.' GROUP BY l.primaryICDCode ORDER BY total DESC LIMIT 5');
            $stmt->bindParam(':username', $val, PDO::PARAM_STR);
            $stmt->bindParam(':date_entered', $date, PDO::PARAM_STR);           
            $stmt->execute();
            return $stmt->fetchAll();
    ...
    function pmbPerc($date,$condition=":username",$val="1")
    {
        global $conn; 
        $date="%".$date."%"; 
        $stmt = $conn->prepare('SELECT COUNT(DISTINCT primaryICDCode) as total FROM claim_line as l INNER JOIN `claim` as a ON l.mca_claim_id=a.claim_id INNER JOIN coding as c ON l.primaryICDCode=c.diag_code WHERE pmb_code<>"" AND a.date_entered like :date_entered AND '.$condition);
         $stmt->bindParam(':username', $val, PDO::PARAM_STR);
         $stmt->bindParam(':date_entered', $date, PDO::PARAM_STR);   
        $stmt->execute();
