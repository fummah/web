...
foreach ($items as $item) {
    ...
    {
        $customers=User::where('role','=','Ordinary')->select(['id', 'name'])->get();
        return $customers;
    }
...
$customer_id=(int)$row['id'];        
   $data[] = array( 
      ...
      "actions"=>"<a type='button' href='#' data='$customer_id'  class='btn btn-success btn-icon btn-sm edit-customer' title='Edit'><i class='now-ui-icons design-2_ruler-pencil'></i></a>
                  <a type='button' href='customer_details/$customer_id' class='btn btn-warning btn-icon btn-sm' title='View Details'><i class='now-ui-icons design_bullet-list-67'></i></a>",     
   );
}
